/*
* Construído por Pato. - KillovSky para Legião Z
...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
		...
		case 'about':
			await kill.sendFile(from, './lib/media/img/alexa.png', 'alexa.png', sobre, id)
			break
       ...
       case 'translate':
        ...
        case 'speak':
			...
			try {
				const spalexa = await axios.get(`http://simsumi.herokuapp.com/api?text=${body.slice(7)}&lang=pt`)
				const a = spalexa.data.success
				if (a == '') {
        ...
        case 'alexa':
			try {
				const alexa = await axios.get(`http://simsumi.herokuapp.com/api?text=${body.slice(6)}&lang=pt`)
				if (alexa.data.success == '') {
					console.log('Request falhou, usando respostas locais...')
					...
					await kill.reply(from, alexa.data.success, id)
				}
			} catch (error) {
					console.log('Request falhou, usando respostas locais...')
					let rndrl = fs.readFileSync('./lib/config/reply.txt').toString().split('\n')
       ...
       case 'clima':
        ...
        case 'ownergroup':
            if (!isGroupMsg) return kill.reply(from, mess.error.Gp, id)
            const _ = chat.groupMetadata.owner
        ...
        case 'mutepv':
            if (isOwner) {
				if (args[0] == 'on') {
					...
					slce.push(pvmt)
					fs.writeFileSync('./lib/config/silence.json', JSON.stringify(slce))
					await kill.reply(from, 'Ele não poderá usar a alexa.', id)
				} else if (args[0] == 'off') {
					if (args.length == 0) return kill.reply(from, 'Você deve definir [on e off] e em seguida o número da pessoa sem - ou +.', id)
					const pvmt = body.slice(11) + '@c.us'
					...
					slce.splice(pvtnm, 1)
					fs.writeFileSync('./lib/config/silence.json', JSON.stringify(slce))
					await kill.reply(from, 'Ele poderá usar a alexa novamente.', id)
				} else {
